...
* **Typography:** Configure `tailwind.config.js` to use `font-serif` for headings (Instrument Serif) and `font-sans` for body (Inter), as defined in `design-tokens.json`. Apply `font-serif` to all Heading elements (h1-h6).
